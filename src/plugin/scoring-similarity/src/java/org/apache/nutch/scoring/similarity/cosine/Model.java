...
public class Model {
	...
	public static synchronized void createModel(Configuration conf) throws IOException {
		...
		try {
			...
			// Check for WordNet databse
			if (!keywordSet.isEmpty()) {
				SynonymsHelper.setPath(conf);
				...
				for (String eachKeyword : keywordSet) {
					...
					for (String eachSynonym : synonymList) {
						...
						LOG.debug(eachSynonym + " " + eachKeyword);
					}
				}
			}

			// TODO : Allow for corpus of documents to be provided as gold
			// standard.
			String line;
	 ...
	 * stage of the crawl cycle to create a DocVector of the currently parsed
	 * page from the parseText attribute value
	 * 
	 * @param content
	 * @param ngram
	 */
	public static DocVector createDocVector(String content, int ngram) {
		...
		if (ngram > 1) {
			...
			tokenizer = new LuceneTokenizer(content, TokenizerType.STANDARD, true, StemFilterType.PORTERSTEM_FILTER);
		}
